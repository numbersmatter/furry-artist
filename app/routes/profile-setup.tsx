...
export async function action({ params, request }: ActionArgs) {
  ...
  if (!inputCheck.success) {
    ...
    const profileNameRaw = inputCheck.data.trim();
...
export default function ProfileSetup() {
  ...
  return (
    <div className="min-h-screen bg-[#2a9bb5] w-full flex flex-col items-center">
          ...
          <SectionPanel name={sectionData.name} text={sectionData.text}>
            {
              sectionData.fields.map((field) => {
                ...
                const errorText = errorObj.errorData[0] 
                ? errorObj.errorData[0].message
                : ""

                 const takenText = "Sorry this name is taken. Please try another one."

                 const userFeedbackText = errorObj.taken ? takenText : errorText
